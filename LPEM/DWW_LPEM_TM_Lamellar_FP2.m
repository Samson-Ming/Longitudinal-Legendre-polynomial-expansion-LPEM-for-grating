...
% ----------------------
N=25;     %Truncation order
L=100;        % Number of layers in a unit cell (also number of points in
             % which field profile is calculated).
Nperiod = 1; % Number periods in which field profile is plotted.
M=5;        % Number of polynomial terms in each layer.
...
eps3=2.25;    %Permittivity of transmission medium
alpha=30*pi/180+1e-9;% Angle of incidence (in radian)
%alpha=30*pi/180;% Angle of incidence (in radian)
...
realEx=real(Ex);
realEz=real(Ez);
normE=abs(Ex).^2+abs(Ez).^2;
figure(2) 
imagesc(x,zz,normE.');shading interp
colorbar
hold on
plot([0,0.25 0.25 0.75 0.75 1],[1 1 0 0 1 1]+1,'r');
hold off
figure(1)
subplot(3,1,1)
