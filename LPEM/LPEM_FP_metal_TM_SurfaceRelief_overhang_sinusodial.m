...
N=20;     %Truncation order, for a constant L, N should not be too large.
L=500;       % Number of layers in a unit cell.
...
% --------------------------------------------------
[Hy Ex Ez DE1 DE3 x zz]=FP_TM(Nperiod,xt2,epst2,sum(dl),lambda,alpha,eps1,eps3,N,M,L,nlayer);
% --------------------------------------------------
absHy=abs(Hy);
absH=absHy;
absEx=abs(Ex);
...
filename = 'Near_field_Metal_overhang_relief_sinusodial_0.5h_Phi=60_TM_1percent_LPEM_N=41_L=500_3_1.mat';
save(filename)     