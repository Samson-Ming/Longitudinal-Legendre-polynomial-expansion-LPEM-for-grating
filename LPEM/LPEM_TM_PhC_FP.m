...
% is recommended to use LPEM_TM_PhC.
...
L=300;       % Number of layers in a unit cell.
Nperiod = 10;
...
lambda=4.0153;     %Vacuum wavelength (Normalized to grating period)
eps1=3.4^2;     %permittivity of incident region
eps3=3.4^2;     %permittivity of transmission region
alpha = 16.87*pi/180+1e-6;% Angle of incidence (in radian)
% ------------------------------------------
...
layertype=[3 4 3];  %1--> homogenous, 2--> square lattice, 3--> triangular lattice
...
radi=[.3 0 .3]; %Rod's radius
epsrod=[1 1 1]*1;   %Rod's permittivity
epssub=[1 1 1]*3.4^2;  %Substrate's permittivity
d_sublayer=[1 1 1]*3^0.5;  %thickness of a sublayer of main layers (Normalized to grating period)
...
% --------------------------------------------------
[Hy Ex Ez DE1 DE3 x zz]=FP_TM(Nperiod,xt,epst,dtotal,lambda,alpha,eps1,eps3,N,M,L,nlayer);
% --------------------------------------------------
absHy=abs(Hy);
absEx=abs(Ex);
absEz=abs(Ez);
