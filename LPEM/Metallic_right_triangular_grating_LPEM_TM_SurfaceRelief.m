...
lambda=0.7;     %Vacuum wavelength (Normalized to the grating period)
d=1;      % Grating thickness (Normalized to the grating period)
eps1=1;     %Permittivity of incident medium
eps3=-21-60.4i;     %Permittivity of transmission medium
%eps3=(1-5i)^2;     %Permittivity of transmission medium
alpha=25*pi/180;% Angle of incidence (in radian)
...
N_l    =100;           % lower number of layers
...
TSvec=[];
 ...
 delta=1e-6;    
[xt,epst]=trapezoidalsurface15(0,1-delta,1-delta,1,eps1,eps3,d,L*nlayer);
%{
xt=xt+0.5;
zlen=length(xt(1,:));
for iii=1:zlen
...
%filename = 'Metallic_right_triangular_relief_trapezoid_TM_1percent_LPEM.mat';
filename = 'Metallic_right_triangular_relief_trapezoid_TM_About1percent_LPEM_N=101_L=100_delta=1em6.mat';
save(filename)       
%{
display(PhaseR)
display(Reflection)
